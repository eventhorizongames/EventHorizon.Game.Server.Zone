namespace EventHorizon.Zone.System.Server.Scripts.Tests.Load
{
    ...
    public class LoadServerScriptsCommandHandlerTests
    {
        ...
        public async Task ShouldSendSetEventWhenOnProcessFileIsRanForFoundFileInfo()
        {
            ...
            var systemsOnProcessFile = default(Func<
                StandardFileInfo,
            ...
            var serverScriptsPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Scripts");
            var loadedScriptsPath = Path.Combine(serverScriptsPath, "Loaded");
            var loadedScriptFileName = "Script.csx";
            var loadedScriptFileFullName = Path.Combine(loadedScriptsPath, loadedScriptFileName);
            ...
            var systemsPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Systems_Data");
            var systemsScriptsPath = Path.Combine(systemsPath, "Wizard", "Wizards");
            var systemsScriptFileName = "Script.csx";
            var systemsScriptFileFullName = Path.Combine(systemsScriptsPath, systemsScriptFileName);
            ...
            var expectedServerScript = new SetServerScriptDetailsCommand(
                new ServerScriptDetails(expectedFileName, "Loaded", loadedScriptFileContent)
            );
            var systemsFileName = systemsScriptFileName.Replace(".csx", string.Empty);
            var systemsFilePath = Path.Combine("Wizard", "Wizards");
            var expectedSystemScript = new SetServerScriptDetailsCommand(
                new ServerScriptDetails(systemsFileName, systemsFilePath, loadedScriptFileContent)
            );
            ...
            serverInfoMock.Setup(mock => mock.ServerScriptsPath).Returns(serverScriptsPath);
            ...
            mediatorMock
                .Setup(
                    mock =>
                        mock.Send(
                            new ReadAllTextFromFile(loadedScriptFileFullName),
                            CancellationToken.None
                        )
                )
                .ReturnsAsync(loadedScriptFileContent);

            mediatorMock
                .Setup(
                    mock =>
                        mock.Send(
                            new ReadAllTextFromFile(systemsScriptFileFullName),
                            CancellationToken.None
                        )
                )
                .ReturnsAsync(loadedScriptFileContent);
            ...
            await handler.Handle(new LoadServerScriptsCommand(), CancellationToken.None);
            onProcessFile.Should().NotBeNull();

            await onProcessFile(loadedScriptFileInfo, arguments);
            await systemsOnProcessFile(systemsScriptFileInfo, systemsArguments);

            // Then
            mediatorMock.Verify(mock => mock.Send(expectedServerScript, CancellationToken.None));
            mediatorMock.Verify(mock => mock.Send(expectedSystemScript, CancellationToken.None));
            detailsRepositoryMock.Verify(mock => mock.Clear());
        }
        ...
        public async Task ShouldNotSendSetCommandWhenExtensionIsNotCSX()
        {
            ...
            var onProcessFile = default(Func<StandardFileInfo, IDictionary<string, object>, Task>);
            var arguments = default(IDictionary<string, object>);
            var systemOnProcessFile = default(Func<
                StandardFileInfo,
                IDictionary<string, object>,
                Task
            >);
            ...
            mediatorMock.Verify(
                mock =>
                    mock.Send(It.IsAny<SetServerScriptDetailsCommand>(), CancellationToken.None),
                Times.Never()
            );
        }
    }
