...
public class DeleteClientAssetCommandHandlerTests
{
    ...
    )
    {
        clientAssetRepositoryMock
            .Setup(mock => mock.Get(request.Id))
            .Returns(clientAsset);
    ...
    )
    {
        ...
        clientAssetRepositoryMock.Verify(
            mock => mock.Delete(It.IsAny<string>()),
            Times.Never()
        );
    }
    ...
    {
        var createBackupOfFileCommand =
            new CreateBackupOfFileCommand(
                assetFileFullName
            );
        var deleteFile = new DeleteFile(assetFileFullName);

        clientAsset.SetFileFullName(assetFileFullName);
