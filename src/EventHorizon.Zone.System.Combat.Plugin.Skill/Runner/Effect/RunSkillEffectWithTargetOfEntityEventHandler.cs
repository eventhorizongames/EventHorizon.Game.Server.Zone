    ...
    : INotificationHandler<RunSkillEffectWithTargetOfEntityEvent>
{
        ...
        foreach (var nextEffect in effect.Next ?? Array.Empty<SkillEffect>())
        {
            ...
            await _mediator.Publish(
                new AddServerActionEvent(
                    _dateTime.Now.AddMilliseconds(effect.Duration),
                    new RunSkillEffectWithTargetOfEntityEvent
                    {
                        ConnectionId = connectionId,
    ...
    )
    {
        ...
        var validationResponseList = await _mediator.Send(
            new RunSkillValidation(skill, effect.ValidatorList, caster, target, targetPosition)
        );
        ...
        return new SkillValidatorResponse { Success = true };
    }
    ...
    {
        var scriptResponse = await _mediator.Send(
        ...
        if (scriptResponse is not SkillEffectScriptResponse effectResponse)
        {
            throw new ArgumentException(
                "Expected a SkillEffectScriptResponse, got: " + scriptResponse.GetType().Name
            );
        }
        ...
        foreach (var clientAction in effectResponse.ActionList)
        {
            ...
            {
                await _mediator.Publish(ClientActionRunSkillActionEvent.Create(clientEvent));
            }
            else if (
                clientAction is ClientActionRunSkillActionForConnectionEvent clientEventToSingle
            )
            {
                await _mediator.Publish(
                    ClientActionRunSkillActionToSingleEvent.Create(clientEventToSingle)
                );
            }
        }
