namespace EventHorizon.Zone.System.Interaction.Run
{
    ...
    public class RunInteractionCommandHandler : IRequestHandler<RunInteractionCommand>
    {
        ...
        public RunInteractionCommandHandler(IMediator mediator)
        {
            _mediator = mediator;
        }

        public async Task Handle(RunInteractionCommand request, CancellationToken cancellationToken)
        {
            var interactionEntity = await _mediator.Send(
                new GetEntityByIdEvent { EntityId = request.InteractionEntityId }
            );
            ...
            if (!interactionState.Active || interactionState.List.IsNull())
            {
                await PublishMessageToPlayer(
                    player.ConnectionId,
            ...
            foreach (var interactionItem in interactionState.List)
            {
                await _mediator.Send(
                    new RunInteractionScriptCommand(interactionItem, interactionEntity, player)
                );
            }
        }
        ...
        )
        {
            await _mediator.Publish(
                SingleClientActionMessageFromCombatSystemEvent.Create(
                    connectionId,
                    new MessageFromCombatSystemData { MessageCode = messageCode, Message = message }
                )
            );
        }
    }
